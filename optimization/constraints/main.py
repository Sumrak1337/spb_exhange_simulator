...
class MainConstraints(Builder):
    ...
    def build(self) -> None:
        ...
        def material_balance_constraint(model, s, n):
            supply = (
                model.supply[s, n]
                if (s, n) in model.supply
                else pyo.ZeroConstant
            )
            demand = (
                model.demand[s, n]
                if (s, n) in model.demand
                else pyo.ZeroConstant
            )
            ...
            if (s, n) in model.transport_to:
                transport_to += sum(
                    model.transport[s, n1, n]
                    for n1 in model.transport_to[s, n]
                )

            transport_from = pyo.ZeroConstant
            if (s, n) in model.transport_from:
                transport_from += sum(
                    model.transport[s, n, n2]
                    for n2 in model.transport_from[s, n]
                )
            ...
            return (
                supply + transport_to + distress_purchase
                == demand + transport_from + distress_sale
            )
