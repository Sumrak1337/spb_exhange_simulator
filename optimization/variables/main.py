...
class MainVariables(Builder):
    ...
    def build(self) -> None:
        ...
        self.model.supply = pyo.Var(self.model.supply_set, within=self.positive_variable, bounds=self._supply_bounds)
        self.model.demand = pyo.Var(self.model.demand_set, within=self.positive_variable, bounds=self._demand_bounds)
        self.model.transport = pyo.Var(self.model.transport_set, within=self.positive_variable, bounds=self._transport_bounds)
    ...
    @staticmethod
    def _supply_bounds(model, *indices: Tuple[AnyStr]) -> Tuple[Numeric, Numeric]:
        return model.supply_min[indices], model.supply_max[indices]

    @staticmethod
    def _demand_bounds(model, *indices: Tuple[AnyStr]) -> Tuple[Numeric, Numeric]:
        return model.demand_min[indices], model.demand_max[indices]

    @staticmethod
    def _transport_bounds(model, *indices: Tuple[AnyStr]) -> Tuple[Numeric, Numeric]:
